...
from .utils import calculate_distance  # put haversine here

# class PunchInQRView(APIView):
#     permission_classes = [IsAuthenticated]
...
#             return Response({"error": "QR token and GPS coordinates required"}, status=400)
...
#             return Response({"error": "Invalid or expired QR"}, status=400)

#         # 2. Validate GPS
#         distance = calculate_distance(
#             float(latitude), float(longitude),
#             qr_session.office.latitude, qr_session.office.longitude
#         )

#         if distance > qr_session.office.allowed_radius:
#             return Response({"error": "You are not within office premises"}, status=403)
...
#                 "location": qr_session.office.name,
...
#                 attendance.save()
#                 return Response({"success": True, "message": "Punch Out successful"})
#             else:
#                 return Response({"error": "Attendance already marked for today"}, status=400)

#         return Response({"success": True, "message": "Punch In successful"})
...
#         try:
#             # Read the uploaded selfie image into a NumPy array
#             selfie_bytes = np.frombuffer(image_file.read(), np.uint8)
#             selfie_image = cv2.imdecode(selfie_bytes, cv2.IMREAD_COLOR)

#             # Read the profile picture from disk
#             profile_image = cv2.imread(employee.profile_pic.path)
...
#                 "confidence": round(result.get('distance', 1.0) * 100, 2)
#             }, status=401)
...
#             "success": True,
#             "message": "Face matched and check-in recorded",
#             "confidence": round((1 - result.get('distance', 0.0)) * 100, 2),
#             "employee": EmployeeSerializer(employee).data,
#             "user": UserLoginSerializer(user).data,
#             "tokens": {
#                 "refresh": str(refresh),
...
def punch_in_view(request):
    ...
    # Get latitude with proper null checking
    lat_value = data.get("latitude") or data.get("phone_lat")
    if lat_value is None:
        return JsonResponse({"status": "failed", "message": "Latitude is required"}, status=400)
    phone_lat = float(lat_value)
    
    # Get longitude with proper null checking
    lon_value = data.get("longitude") or data.get("phone_lon")
    if lon_value is None:
        return JsonResponse({"status": "failed", "message": "Longitude is required"}, status=400)
    ...
    # Check if there's an active punch-in (punch-in without punch-out) for today
    active_punch_in = Attendance.objects.filter(
        employee=employee, 
        date=today, 
        out_time__isnull=True,
    ...
    if active_punch_in:
        return JsonResponse({
            "status": "failed", 
            "message": "You have an active punch-in session. Please punch-out first before punching in again."
        }, status=400)

    # Count total punch-ins for today to track multiple sessions
    today_punch_count = Attendance.objects.filter(employee=employee, date=today).count()
    ...
    return JsonResponse({
        ...
        "punch_session": today_punch_count + 1,  # Current session number
...
class EmployeeAttendanceView(APIView):
    ...
    def get(self, request):
        user = request.user
        employee = user.employee_profile
        attendances_qs = Attendance.objects.filter(employee=employee).order_by("-date", "-in_time")
        serializer = AttendanceSerializer(attendances_qs, many=True)
        ...
        for attendance in attendances:
            if attendance.out_time:
                ...
                session_details.append({
                    ...
                    "punch_in": attendance.in_time.strftime("%H:%M:%S"),
                    "punch_out": attendance.out_time.strftime("%H:%M:%S"),
...
# login employees presence , absence , leaves count
class EmployeePresenceAbsenceLeaveCountView(APIView):
    ...
    def get(self, request):
        user = request.user
        employee = user.employee_profile
        presence_count = Attendance.objects.filter(employee=employee, status="Present").count()
        absence_count = Attendance.objects.filter(employee=employee, status="Absent").count()
        leave_count = Leave.objects.filter(user=user).count()
        return Response({"success": True, "presence_count": presence_count, "absence_count": absence_count, "leave_count": leave_count})
...
# To show login employees leave details    
from rest_framework.generics import ListAPIView
...
class EmployeePresenceAbsenceLeaveCountView(APIView):
    ...
    def get(self, request):
        ...
        # Handle leaves
        leaves = Leave.objects.filter(user=user).values("start_date", "end_date")
        for leave in leaves:
